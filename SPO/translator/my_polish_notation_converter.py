...
class PN:
    ...
    def __while_pn(self):
        _output = []
        self.token_count += 2
        _output += self.__expr_pn('CP')
        _output.append(Token('GO_F', ''))
        self.token_count += 2
        while self.tokens[self.token_count].get_type() not in 'CB':
            _output += self.__expr_pn('SEMICOLON')
            self.token_count += 1
        for o in _output:
            if o.get_type() == 'GO_F':
        ...
        while self.tokens[self.token_count].get_type() not in stop_token:
            ...
            if self.tokens[self.token_count].get_type() == 'VAR' or self.tokens[self.token_count].get_type() == 'DIGIT':
                ...
                if not _stack:
                    ...
                    while get_priority(_stack[-1].get_value()) >= get_priority(
