...
class Triad:
    ...
    def __triads_to_pm(self):
        for i in range(len(self.triads)):
            if self.triads[i][3].get_type() == 'CONST':
                pass
            elif self.triads[i][3].get_type() == 'WHILE' or self.triads[i][3].get_type() == 'IF' or self.triads[i][
                3].get_type() == 'GO_F' or self.triads[i][3].get_type() == 'GO_A' or self.triads[i][3].get_type() == 'END':
                self.pn.append(self.triads[i][3])
            else:
                if not self.__found_triad_link(self.triads[i]):
                    if self.triads[i][1].get_type() != 'TR':
                        self.pn.append(self.triads[i][1])
                    else:
                        self.pn += self.__add_triad(self.triads[i][1])
                    if self.triads[i][2].get_type() != 'TR':
                        self.pn.append(self.triads[i][2])
                    else:
                        self.pn += self.__add_triad(self.triads[i][2])
                    self.pn.append(self.triads[i][3])
        self.pn = [i for i in self.pn if i.get_type() != 'FUN' and i.get_value() != 'RETURN']
        self.__change_transitions()

    def __change_transitions(self):
        self.token_count = 0
        while self.token_count < len(self.pn):
            if self.pn[self.token_count].get_type() == 'WHILE':
                self.token_count += 1
                self.out += self.__while_transitions()
            elif self.pn[self.token_count].get_type() == 'IF':
                self.token_count += 1
                self.out += self.__if_transitions()
            else:
                self.out.append(self.pn[self.token_count])
            self.token_count += 1
        pass

    def __while_transitions(self):
        _out = []
    ...
    def __search_index(self, tr):
        for i in range(len(self.triads)):
            if self.triads[i][0] == tr:
                return i

    def __add_triad(self, t):
        ind = self.__search_index(t.get_value())
        out = []
        t = self.triads[ind]
        if t[3].get_type() == 'CONST':
            ...
            if True:
                if t[1].get_type() != 'TR':
                    ...
                    out += self.__add_triad(t[1])
                if t[2].get_type() != 'TR':
                    out.append(t[2])
                else:
                    out += self.__add_triad(t[2])
                out.append(t[3])
        return out
    ...
    def __transfer(self):
        ...
        while self.token_count < len(self.tokens):
            ...
            if self.tokens[self.token_count].get_type() == 'VAR' or self.tokens[self.token_count].get_type() == 'DIGIT':
            ...
            elif self.tokens[self.token_count].get_type() == 'ARI_OP' or self.tokens[
                ...
                self.triads.append(['^' + str(len(self.triads)), Token('GO_F', 'GO_F'), self.tokens[self.token_count],
                                    self.tokens[self.token_count]])
            elif self.tokens[self.token_count].get_type() == 'GO_A':
                self.triads.append(['^' + str(len(self.triads)), Token('GO_A', 'GO_A'), self.tokens[self.token_count],
                                    self.tokens[self.token_count]])
                ...
                list = []
                f_ind = self.__search_fun_index(self.tokens[self.token_count].get_value())
                length = len(self.function[f_ind][1])
                for i in range(length):
                    list.append(self.stack.pop())
                list.reverse()
                self.triads.append(['^' + str(len(self.triads)), Token('FN_VALUE', list), self.tokens[self.token_count],
                                    Token('FUN', 'FUN')])
    ...
    def __optimize(self):
        ...
        while count < len(self.triads):
            ...
            if self.triads[count][3].get_type() == 'ASSIGN_OP':
                ...
                if self.__find_value(self.triads[count][1].get_value()) is None:
                    self.value_table.append([self.triads[count][1].get_value(), operand2])
                else:
                    ind = self.__find_value(self.triads[count][1].get_value(), False)
                ...
                if operand2.get_type() == 'DIGIT' and operand1.get_type() == 'DIGIT':
                    self.triads[count][1] = self.__operation(operand1.get_value(), operand2.get_value(),
    ...
    def __reduction(self):
        ...
        while count < len(self.triads):
            if self.triads[count][3].get_type() == 'CONST':
                if not self.__found_triad_link(self.triads[count], count):
                ...
                if self.triads[count][1].get_value() in self.stop_list:
                    count += 1
                    continue
                c = self.__found_var(self.triads[count], 0, count, False)
                if c[0] and self.triads[c[1]][3].get_type() == 'ASSIGN_OP' and self.triads[c[1]][
                    2].get_type() == 'DIGIT':
                    if not self.__found_var(self.triads[count], c[1] + 1, count):
                        del self.triads[c[1]]
                        count -= 1

                if not self.__found_var(self.triads[count], count + 1, len(self.triads)):
                    del self.triads[count]
                    count -= 1

            count += 1

    def __found_var(self, t, count, fin, fl=True):
